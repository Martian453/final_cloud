    ...
    datasets: [
      {
        ...
        pointBackgroundColor: [
          (activeMetric === "level" || hoveredMetric === "level") ? "rgba(143, 211, 255, 1)" : "rgba(143, 211, 255, 0.4)",
          (activeMetric === "ph" || hoveredMetric === "ph") ? "rgba(124, 255, 154, 1)" : "rgba(124, 255, 154, 0.4)",
          (activeMetric === "turbidity" || hoveredMetric === "turbidity") ? "rgba(255, 211, 106, 1)" : "rgba(255, 211, 106, 0.4)",
        ],
      ...
      },
      {
        ...
        data: [animatedValues.level, animatedValues.ph, animatedValues.turbidity],
        backgroundColor: [
          (activeMetric === "level" || hoveredMetric === "level") ? "rgba(6, 182, 212, 1)" : "rgba(6, 182, 212, 0.3)", // Cyan-500
          (activeMetric === "ph" || hoveredMetric === "ph") ? "rgba(34, 197, 94, 1)" : "rgba(34, 197, 94, 0.3)", // Green-500
          (activeMetric === "turbidity" || hoveredMetric === "turbidity") ? "rgba(251, 191, 36, 1)" : "rgba(251, 191, 36, 0.3)", // Amber-400 (Yellow)
        ],
        borderRadius: 8,
        barThickness: 50,
    ...
    >
      ...
      <div className="relative z-10 mb-4 grid grid-cols-3 gap-2">
        {metrics.map((m, i) => (
          <div
            key={m.label}
            onClick={() => onMetricSelect && onMetricSelect(m.key === activeMetric ? "" : m.key)}
            className={`group/item cursor-pointer rounded-xl border p-3 text-center backdrop-blur-sm transition-all duration-300 ${(activeMetric === m.key || hoveredMetric === m.key)
              ? `border-cyan-500/50 bg-cyan-500/10 scale-105 ${m.bgGlow}`
              : "border-white/5 bg-slate-900/50 hover:border-cyan-500/30 hover:bg-slate-800/50"
              }`}
