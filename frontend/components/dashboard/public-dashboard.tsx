    ...
    useEffect(() => {
        const fetchPublicData = async () => {
            try {
                ...
                const res = await fetch("http://localhost:8000/api/public/locations")
                const data = await res.json()
                if (Array.isArray(data)) {
                    setLocations(data)

                    if (data.length > 0 && !selectedLoc) {
            ...
            <header className="px-6 py-5 border-b border-white/5 flex items-center justify-between backdrop-blur-md bg-black/20 sticky top-0 z-50">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] uppercase tracking-widest text-slate-500">Public Access Network</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button
                        onClick={() => router.push("/login")} // redirect to login > register
                        className="hidden sm:flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-bold shadow-lg shadow-emerald-500/20 transition-all"
                    >
                        Add Device
                    </button>
                </div>
            ...
            <main className="flex-1 p-6 lg:p-8 max-w-7xl mx-auto w-full">
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {locations.map(loc => (
                            <div key={loc.location_id} className="bg-slate-900/40 border border-white/10 rounded-2xl p-6 hover:bg-white/5 transition-all group">
